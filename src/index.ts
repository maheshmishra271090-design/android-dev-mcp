...
function runShell(fullCommand: string, cwd?: string): { stdout: string; error?: string } {
  try {
    ...
    const stdout = execSync(fullCommand, opts);
...
function binAvailable(bin: string): boolean {
  try { execSync(`which ${bin}`, { encoding: "utf-8" }); return true; }
  catch { return false; }
}

function androidCLIAvailable(): boolean { return binAvailable("android"); }
function adbAvailable(): boolean { return binAvailable("adb"); }
function gitAvailable(): boolean { return binAvailable("git"); }

function gradleInvocation(cwd?: string): string {
